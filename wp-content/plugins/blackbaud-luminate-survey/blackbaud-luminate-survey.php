...
function blackbaud_luminate_survey_init($blackbaud) {
    ...
    function survey_cpt($plugin, $blackbaud) {
        ...
        return array(
			...
			'description' => __('Allows you to easily create and store various Luminate Online Surveys.', $domain),
			'supports'    => array('title'),
			'menu_icon'   => 'dashicons-clipboard',
			'labels'      => array(
				'name'               => __('Surveys', $domain),
				'singular_name'      => __('Survey', $domain),
				'menu_name'          => _x('Surveys', 'admin menu', $domain),
				'name_admin_bar'     => _x('Survey', 'add new on admin bar', $domain),
				'add_new'            => _x('Add New', 'survey', $domain),
				'add_new_item'       => __('Add New Survey', $domain),
				'new_item'           => __('New Survey', $domain),
				'edit_item'          => __('Edit Survey', $domain),
				'view_item'          => __('View Survey', $domain),
				'all_items'          => __('All Surveys', $domain),
				'search_items'       => __('Search Surveys', $domain),
				'parent_item_colon'  => __('Parent Surveys:', $domain),
				'not_found'          => __('No surveys found.', $domain),
				'not_found_in_trash' => __('No surveys found in Trash.', $domain)
			)
		);
    }
    ...
    function survey_metabox($plugin, $blackbaud) {
        ...
        return array(
            ...
            'fields'    => array(
				array(
					'slug'       => 'survey_id',
					'label'      => __('Luminate Online Survey ID:', $domain),
					'helplet'    => __("This ID is set by Luminate Online and found beneath the Survey's title after you login.", $domain),
					'type'       => 'text',
					'attributes' => array(
						'class'       => 'form-control',
						'maxlength'   => '10',
        ...
        return array(
            ...
            'columns' => array(
                'luminate_survey_id' => array(
                    'label' => __('Survey ID', $domain),
                    'order_by_meta' => 'survey_id',
                    'is_integer' => true,
    ...
    function survey_shortcode($plugin, $blackbaud) {
        return array(
            ...
            'output' => function ($data) use ($plugin, $blackbaud) {
                return $plugin->get_template("survey.blackbaud-luminate-survey.php", $data);
            }
        );
    }
    ...
    function luminate_extend($plugin, $blackbaud) {
        ...
        $plugin->expose_setting('api', array(
            ...
            'handlebarsTemplate' => json_encode($plugin->get_template('survey.hbs'))
        ));
